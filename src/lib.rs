...
pub trait BitOps:BitTypes {
    ...
    fn bitmask<R:NumRangeExtract<Self::TYPE> >(range: &R) -> Self;
    ...
    fn get_bits<R:NumRangeExtract<Self::TYPE> >(&self, range:&R) -> Self;
    /// count 0 bits in range (0 indexed)
    fn ctz<R:NumRangeExtract<Self::TYPE> >(&self, range:&R) -> usize;
    /// count 1 bits in range (0 indexed)
    fn popcnt<R:NumRangeExtract<Self::TYPE> >(&self, range:&R) -> usize;
    /// set bits in range to val (0 indexed)
    fn set_bits<R:NumRangeExtract<Self::TYPE> >(&mut self, range:&R, val:bool);
    /// set all bits to val
    fn set_all_bit(val:bool) -> Self;
    /// set a specfic range of self to these bits (0 indexed)
    fn set_these_bits<R:NumRangeExtract<Self::TYPE> >(&mut self, bits:Self, range:&R);
    ...
    ($($type:ty),*) => {
        $(
            ...
            impl BitOps for $type {
                ...
                fn bitmask<R:NumRangeExtract<Self::TYPE>>(range:&R) -> Self { //indexes: 0..=Self::BITS-1
                ...
                fn get_bits<R:NumRangeExtract<Self::TYPE> >(&self, range:&R) -> Self {Self::bitmask(range) & self}
                fn popcnt<R:NumRangeExtract<Self::TYPE> >(&self, range:&R) -> usize {
